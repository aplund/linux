...
static int meson_vrtc_set_alarm(struct device *dev, struct rtc_wkalrm *alarm)
{
	...
	if (alarm->enabled) {
		...
		/* alarm_secs = rtc_tm_to_time64(&alarm->time); */
		/* ktime_get_raw_ts64(&time); */
		...
		/* dev_dbg(dev, "system will wakeup in %lus.\n", alarm_secs); */
	} else {
		vrtc->alarm_time = 0;
...
#endif
static SIMPLE_DEV_PM_OPS(meson_vrtc_pm_ops, meson_vrtc_suspend, meson_vrtc_resume);
